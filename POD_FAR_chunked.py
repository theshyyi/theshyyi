...
CHUNKS = {"time": 90, "lat": 200, "lon": 200}
...
def open_dataset_maybe_chunked(path: str, chunks: dict | None = None) -> xr.Dataset:
    ...
    """
    if chunks is None:
        return open_dataset_maybe_chunked(path, CHUNKS)
    try:
        import dask  # noqa: F401
    except Exception:
        # dask not installed; open without chunking
        return open_dataset_maybe_chunked(path, CHUNKS)
...
def build_china_mask(ref_ds: xr.Dataset, shp_path: str) -> xr.DataArray | None:
    ...
    # regionmask API differs across versions.
    # Newer versions: Regions.mask(ds) works (infers lon/lat names).
    # Older versions: Regions.mask(lon, lat) (no lon_name/lat_name kwargs).
    # We support both.
    def _guess_lon_lat_names(ds: xr.Dataset) -> Tuple[str, str]:
        lon_cands = ["lon", "longitude", "LONGITUDE", "x"]
        lat_cands = ["lat", "latitude", "LATITUDE", "y"]
        lon_name = next((n for n in lon_cands if n in ds.coords or n in ds.variables), None)
        lat_name = next((n for n in lat_cands if n in ds.coords or n in ds.variables), None)
        if lon_name is None or lat_name is None:
            raise KeyError(
                "Cannot infer lon/lat coordinate names from dataset. "
                f"Found coords={list(ds.coords)}"
            )
        return lon_name, lat_name

    try:
        mask = reg.mask(ref_ds)
    except TypeError:
        lon_name, lat_name = _guess_lon_lat_names(ref_ds)
        mask = reg.mask(ref_ds[lon_name], ref_ds[lat_name])
...
def compute_all_stats(
    ...
    for prod, path in product_paths.items():
        ...
        sim_ds = open_dataset_maybe_chunked(path, CHUNKS)
